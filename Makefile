...
image-build:
	make build
	docker build -t ghcr.io/feelpp/dropbox-csi:canary .
clean:
	go clean ./...
	rm -rf build/
test-cluster-up:
...
yaml-deploy:
	...
	sed -i 's\ghcr.io/feelpp/dropbox-csi:latest\ghcr.io/feelpp/dropbox-csi:canary\' /tmp/csi-dropbox-plugin.yaml
